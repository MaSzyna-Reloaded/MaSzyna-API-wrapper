...
namespace godot {
    ...
    void VehicleBrake::alarm_chain(const bool p_pulled) {
    ...
    void VehicleBrake::brake_releaser(const bool p_pressed) {
        TMoverParameters *mover = get_mover();
        ASSERT_MOVER_BRAKE(mover);
        mover->BrakeReleaser(p_pressed ? 1 : 0);
    }

    void VehicleBrake::brake_level_set(const double p_level) {
        TMoverParameters *mover = get_mover();
        ASSERT_MOVER_BRAKE(mover);
    ...
    void VehicleBrake::brake_level_set_position(const BrakeHandlePosition p_position) {
        TMoverParameters *mover = get_mover();
        ASSERT_MOVER_BRAKE(mover);
    ...
    void VehicleBrake::brake_level_set_position_str(const String &p_position) {
        TMoverParameters *mover = get_mover();
        ASSERT_MOVER_BRAKE(mover);
    ...
    void VehicleBrake::brake_level_increase() {
        TMoverParameters *mover = get_mover();
        ASSERT_MOVER_BRAKE(mover);
        mover->IncBrakeLevel();
    }

    void VehicleBrake::brake_level_decrease() {
        TMoverParameters *mover = get_mover();
        ASSERT_MOVER_BRAKE(mover);
        mover->DecBrakeLevel();
    }
    ...
    void VehicleBrake::local_brake_set(const double p_level) {
        TMoverParameters *mover = get_mover();
        ASSERT_MOVER_BRAKE(mover);
    ...
    void VehicleBrake::local_brake_increase() {
        TMoverParameters *mover = get_mover();
        ASSERT_MOVER_BRAKE(mover);
    ...
    void VehicleBrake::local_brake_decrease() {
        TMoverParameters *mover = get_mover();
        ASSERT_MOVER_BRAKE(mover);
    ...
    void VehicleBrake::manual_brake_increase() {
        TMoverParameters *mover = get_mover();
        ASSERT_MOVER_BRAKE(mover);
    ...
    void VehicleBrake::auto_rewident(const int p_brake_delay) {
        TMoverParameters *mover = get_mover();
        ASSERT_MOVER_BRAKE(mover);
    ...
    void VehicleBrake::brake_level_charging(const bool p_active) {
        TMoverParameters *mover = get_mover();
        ASSERT_MOVER_BRAKE(mover);
        if (p_active) {
    ...
    void VehicleBrake::manual_brake_decrease() {
        TMoverParameters *mover = get_mover();
        ASSERT_MOVER_BRAKE(mover);
        if (mover->LocalBrake == TLocalBrake::ManualBrake || mover->MBrake) {
    ...
    void VehicleBrake::_do_process_mover(TMoverParameters *p_mover, const double p_delta) {
    ...
    void VehicleBrake::_do_update_internal_mover(TMoverParameters *p_mover) {
        ...
        for (int i = 0; i < brake_pressure_table.size(); i++) {
            ...
            if (row == nullptr || !row.is_valid()) {
                UtilityFunctions::push_warning(
                        "[VehicleBrake]: brake_pressure_table property is null at index " + String::num(i));
                continue;
            }
        ...
        if (compressor_list_size > MAX_COMPRESSOR_LIST) {
            UtilityFunctions::push_warning(
                    "[VehicleBrake]: compressor_list has " + String::num(compressor_list_size) +
        ...
        for (int i = 0; i < std::min(MAX_COMPRESSOR_LIST, compressor_list_size); i++) {
            ...
            if (row == nullptr || !row.is_valid()) {
                UtilityFunctions::push_warning(
                        "[VehicleBrake]: compressor_list property is null at index " + String::num(i));
                continue;
            }
