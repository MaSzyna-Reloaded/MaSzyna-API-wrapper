...
func test_defaults():
    engine.update_mover()
    assert_eq(engine.get("min_rpm"), 0.0)
    assert_eq(engine.get("max_rpm"), 0.0)
    assert_eq(engine.get("inertia"), 1.0)
    assert_false(engine.get("has_torque_converter"))
    assert_false(engine.get("has_retarder"))
    assert_eq(engine.get("retarder_placement"), TrainDieselEngine.RETARDER_PLACEMENT_AFTER_GEARBOX)
    assert_eq((engine.get("throttle_table") as Array).size(), 0)
    assert_eq(engine.torque_table.size(), 0)
    assert_eq((engine.get("torque_converter_table") as Array).size(), 0)
    assert_true(train.config.get("engine_shake_enabled", false))

func test_mechanical_and_torque_converter_round_trip():
    engine.set("min_rpm", 600.0)
    engine.set("max_rpm", 2000.0)
    engine.set("fuel_cutoff_rpm", 2100.0)
    engine.set("inertia", 1.5)
    engine.set("clutch_engage_speed", 0.6)
    engine.set("clutch_disengage_speed", 0.8)
    engine.set("has_torque_converter", true)
    engine.set("tc_max_torque_ratio", 2.5)
    engine.set("tc_coupling_point", 0.9)
    engine.set("has_retarder", true)
    engine.set("retarder_placement", TrainDieselEngine.RETARDER_PLACEMENT_BETWEEN_GEARBOX_AND_TC)
    engine.set("retarder_max_torque", 500.0)
    engine.set("torque_converter_table", [_make_point(0.0, 4.89), _make_point(1.0, 0.0)])
    await wait_idle_frames(2)

    assert_eq(engine.get("min_rpm"), 600.0)
    assert_eq(engine.get("max_rpm"), 2000.0)
    assert_true(engine.get("has_torque_converter"))
    assert_eq(engine.get("tc_max_torque_ratio"), 2.5)
    assert_true(engine.get("has_retarder"))
    assert_eq(engine.get("retarder_placement"), TrainDieselEngine.RETARDER_PLACEMENT_BETWEEN_GEARBOX_AND_TC)
    assert_eq((engine.get("torque_converter_table") as Array).size(), 2)
...
func test_throttle_table_and_torque_curve_round_trip():
    engine.set("max_torque", 1400.0)
    engine.set("max_torque_rpm", 1200.0)
    engine.set("nominal_fuel_dose", 1.0)
    engine.set("nominal_fuel_consumption_rate", 210.0)
    engine.set("throttle_table", [
    ...
    var throttle_table: Array = engine.get("throttle_table")
...
func test_oversized_throttle_table_is_truncated_without_crashing():
    ...
    engine.set("throttle_table", rows)
    await wait_idle_frames(2)
