...
func test_filename_changes_debounce_and_release_previous_content() -> void:
    ...
    for cycle:int in range(3):
        ...
        scenery.filename = "intermediate.scn"
        scenery.filename = "next_%s.scn" % cycle
        await wait_idle_frames(3)
        assert_eq(scenery.imports, cycle + 1, "filename changes wait for debounce")
        await get_tree().create_timer(0.6).timeout
    ...
    scenery.filename = scenery.filename
    await get_tree().create_timer(0.6).timeout
    assert_eq(scenery.imports, 4, "assigning the same filename does not reload")
