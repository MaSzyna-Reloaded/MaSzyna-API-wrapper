...
namespace godot {
    namespace {
        ...
        std::vector<TrainController *> get_sorted_active_train_controllers() {
            std::vector<TrainController *> controllers(
                    active_train_controllers.begin(), active_train_controllers.end());
            std::sort(
    ...
    void TrainController::_bind_methods() {
        ...
        BIND_PROPERTY(
                Variant::FLOAT, "drag_coefficient", "drag_coefficient", &TrainController::set_drag_coefficient,
                &TrainController::get_drag_coefficient, "drag_coefficient");
        ...
        ADD_SIGNAL(MethodInfo(
                DERAILED_SIGNAL, PropertyInfo(Variant::INT, "damage_flag"),
                PropertyInfo(Variant::INT, "derail_reason")));
    ...
    void TrainController::_do_fetch_state_from_mover(TMoverParameters *mover, Dictionary &state) {
        ...
        state["front_derail_threshold_self_kmh"] =
                mover->Neighbours[end::front].vehicle != nullptr &&
                                mover->Neighbours[end::front].vehicle->TotalMass > 0.0
                        ? 15.0 * (mover->TotalMass / mover->Neighbours[end::front].vehicle->TotalMass)
                        : 0.0;
        state["front_derail_threshold_other_kmh"] =
                mover->Neighbours[end::front].vehicle != nullptr && mover->TotalMass > 0.0
        ...
        state["rear_neighbour_damage_flag"] =
                mover->Neighbours[end::rear].vehicle != nullptr ? mover->Neighbours[end::rear].vehicle->DamageFlag : 0;
        state["rear_relative_speed_kmh"] = compute_relative_speed_kmh(mover, mover->Neighbours[end::rear]);
        state["rear_derail_threshold_self_kmh"] =
                mover->Neighbours[end::rear].vehicle != nullptr && mover->Neighbours[end::rear].vehicle->TotalMass > 0.0
                        ? 15.0 * (mover->TotalMass / mover->Neighbours[end::rear].vehicle->TotalMass)
                        : 0.0;
        state["rear_derail_threshold_other_kmh"] =
                mover->Neighbours[end::rear].vehicle != nullptr && mover->TotalMass > 0.0
    ...
    void TrainController::_handle_mover_update() {
        ...
        if (!was_derailed && is_derailed) {
            emit_signal(
                    DERAILED_SIGNAL, static_cast<int>(state.get("damage_flag", 0)),
    ...
    void TrainController::refresh_runtime_signals() {
        ...
        const bool new_is_powered =
                (current_state.get("power24_available", false) || current_state.get("power110_available", false));
