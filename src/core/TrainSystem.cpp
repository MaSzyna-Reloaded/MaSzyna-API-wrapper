...
namespace godot {
    ...
    void TrainSystem::register_command(const String &train_id, const String &command, const Callable &callback) {
        ...
        Dictionary registered_trains = static_cast<Dictionary>(commands[command]);
        if (registered_trains.has(train_id)) {
            log(train_id, GameLog::LogLevel::ERROR, "Command is already registered: " + command);
            return;
        }

        registered_trains[train_id] = callback;
        commands[command] = registered_trains;
    }
    ...
    void TrainSystem::unregister_command(const String &train_id, const String &command, const Callable &callback) {
        ...
        registered_trains.erase(train_id);
        if (registered_trains.size() == 0) {
    ...
    void TrainSystem::send_command(const String &train_id, const String &command, const Variant &p1, const Variant &p2) {
        const std::map<String, TrainController *>::iterator it = trains.find(train_id);
        ...
        if (is_command_supported(command)) {
            ...
            if (const Callable callback = registered_trains[train_id]; callback.is_valid()) {
                Array args;
                const int argc = static_cast<int>(callback.get_argument_count());
                if (argc > 0) {
                    args.append(p1);
                }
                if (argc > 1) {
                ...
                if (arg_required != argc) {
                    log(train_id, GameLog::LogLevel::WARNING,
                            "Method " + String(callback.get_object()->get_class()) + "::" +
                                    String(callback.get_method().get_basename()) + " should handle " +
                                    String::num_int64(arg_required) + " arguments, but it has " +
                                    String::num_int64(argc));
                }
#endif
            } else {
                log(train_id, GameLog::LogLevel::ERROR,
                        "Callable " + String(callback.get_method().get_basename()) + " is invalid");
            }
        } else {
