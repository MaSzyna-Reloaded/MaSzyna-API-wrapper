...
func test_defaults():
    assert_eq(engine.cntrl_converter_start_mode, TrainEngine.START_MODE_MANUAL)
    assert_false(engine.cntrl_pantograph_auto_valve)
    assert_eq(engine.cntrl_main_switch_start_mode, TrainEngine.START_MODE_MANUAL)

func test_round_trip_and_update_without_crashing():
    engine.cntrl_converter_start_mode = TrainEngine.START_MODE_AUTOMATIC
    engine.cntrl_converter_start_delay = 2.0
    engine.cntrl_converter_overload_relay_start_mode = TrainEngine.START_MODE_CONVERTER
    engine.cntrl_converter_overload_relay_off_when_main_is_off = true
    engine.cntrl_pantograph_compressor_start_mode = TrainEngine.START_MODE_AUTOMATIC
    engine.cntrl_pantograph_auto_valve = true
    engine.cntrl_main_switch_start_mode = TrainEngine.START_MODE_AUTOMATIC
    await wait_idle_frames(2)

    assert_eq(engine.cntrl_converter_start_mode, TrainEngine.START_MODE_AUTOMATIC)
