...
@export_node_path("E3DModelInstance") var sleeper_e3d: NodePath:
    set(x):
        sleeper_e3d = x
        if is_inside_tree() and _renderer:
            _renderer.set_sleeper_e3d(get_node_or_null(x))
        _dirty = true
...
@export var ballast_enabled: bool = true:
    set(x):
        ballast_enabled = x
        if _renderer: _renderer.ballast_enabled = x
        _dirty = true

@export var ballast_texture: String = "":
    set(x):
        ballast_texture = x
        if _renderer: _renderer.set_ballast_texture(x)
        _dirty = true

@export var ballast_material: ShaderMaterial:
    set(x):
        ballast_material = x
        if _renderer: _renderer.ballast_material = x
        _dirty = true

@export var ballast_uv_scale: float = 1.0:
    set(x):
        ballast_uv_scale = x
        if _renderer: _renderer.ballast_uv_scale = x
        _dirty = true

@export var ballast_width_tiling: float = 1.0:
    set(x):
        ballast_width_tiling = x
        if _renderer: _renderer.ballast_width_tiling = x
        _dirty = true

@export var ballast_length_tiling: float = 1.0:
    set(x):
        ballast_length_tiling = x
        if _renderer: _renderer.ballast_length_tiling = x
        _dirty = true

@export var ballast_height: float = 0.4:
    set(x):
        ballast_height = x
        if _renderer: _renderer.ballast_height = x
        _dirty = true

@export var ballast_offset: float = -0.05:
    set(x):
        ballast_offset = x
        if _renderer: _renderer.ballast_offset = x
        _dirty = true
...
func _ready():
    ...
    if not ballast_material:
        ballast_material = ShaderMaterial.new()
        ballast_material.shader = load("res://addons/libmaszyna/materials/ballast_path_deform.gdshader")
    
    if rail_material:
        rail_material = rail_material.duplicate()
    if sleeper_material:
        sleeper_material = sleeper_material.duplicate()
    if ballast_material:
        ballast_material = ballast_material.duplicate()
    
    _renderer.rail_material = rail_material
    _renderer.sleeper_material = sleeper_material
    _renderer.ballast_material = ballast_material
    _renderer.set_ballast_texture(ballast_texture)
    _renderer.sleeper_height = sleeper_height
    _renderer.sleeper_spacing = sleeper_spacing
    _renderer.rail_spacing = rail_spacing
    _renderer.ballast_height = ballast_height
    _renderer.ballast_offset = ballast_offset
    _renderer.ballast_uv_scale = ballast_uv_scale
    _renderer.ballast_width_tiling = ballast_width_tiling
    _renderer.ballast_length_tiling = ballast_length_tiling
    _renderer.ballast_enabled = ballast_enabled
    _renderer.collision_enabled = collision_enabled
    if sleeper_e3d:
        _renderer.set_sleeper_e3d(get_node_or_null(sleeper_e3d))
...
func _update_track():
    ...
    if points.size() > 0:
        ...
        RenderingServer.instance_set_custom_aabb(_renderer.get_ballast_mesh_instance(), aabb)
...
func _update_ballast_mesh(length: float):
    if _renderer and ballast_enabled:
        _renderer.update_ballast_mesh(length, curve_precision)
