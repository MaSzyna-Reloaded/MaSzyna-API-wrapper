...
func track_free(track_rid: RID) -> void:
    ...
    var track_name:Variant = _named_tracks.find_key(track_rid)
    if track_name:
...
func track_update(
    ...
    if points:
        var rect:Rect2 = Rect2(Vector2(points[0].x, points[0].z), Vector2.ZERO)
...
func track_get_name(track_rid: RID) -> String:
    var track: TrackSegment = _tracks.get(track_rid)
    if track:
        var name:Variant = _named_tracks.find_key(track_rid)
...
func topology_get_summary() -> Dictionary:
    var stats:Dictionary = {"graphs": [], "orphaned_tracks_count": 0}

    # Calculate graphs stats
    for graph_id:int in _graph_members:
        var track_rids:Array[RID] = _graph_members[graph_id]
        var num_tracks:int = track_rids.size()
        var num_switches:int = 0
        var total_length:float = 0.0

        for track_rid:RID in track_rids:
            var track:TrackSegment = _tracks.get(track_rid)
    ...
    for track_rid: RID in _tracks.keys():
        ...
        while queue:
            ...
            if not _graph_members.has(graph_id):
                var graph_tracks:Array[RID] = []
                _graph_members[graph_id] = graph_tracks
