...
func test_param_and_dimensions_defaults():
    ...
    assert_eq(train.get_length(), 0.0)
    assert_eq(train.get_height(), 0.0)
    assert_eq(train.get_width(), 0.0)
    assert_eq(train.get_drag_coefficient(), 0.0)
    assert_eq(train.get_floor_height(), 0.96)
...
func test_param_and_dimensions_round_trip_and_update():
    ...
    train.set_length(15.5)
    train.set_height(4.3)
    train.set_width(2.9)
    train.set_drag_coefficient(1.2)
    train.set_floor_height(1.1)
    train.cabin_count = 2
    await wait_idle_frames(2)
    ...
    assert_eq(train.get_length(), 15.5)
    assert_eq(train.get_height(), 4.3)
    assert_eq(train.get_width(), 2.9)
    assert_eq(train.get_drag_coefficient(), 1.2)
    assert_almost_eq(train.get_floor_height(), 1.1, 0.001)
