...
func test_defaults():
    ...
    assert_eq(switches.get("relay_reset_button_1"), 0)
    assert_false(switches.modern_dimmer)
    assert_eq((switches.get("dimmer_list") as Array).size(), 0)
...
func test_round_trip_and_update_without_crashing():
    ...
    switches.set("relay_reset_button_1", 1 | 2)
    switches.pantograph_presets = PackedInt32Array([0, 1, 3, 2])
    switches.pantograph_preset_default = 1
    switches.modern_dimmer = true
    switches.set("dimmer_list_cycle", false)
    switches.set("dimmer_list_default_position", 3)
    switches.set("dimmer_list", [
    ...
    assert_eq(switches.get("relay_reset_button_1"), 3)
    assert_eq(switches.pantograph_presets.size(), 4)
    var dimmer_list: Array = switches.get("dimmer_list")
