    ...
    */
    ...
    double TMoverParameters::Current(double n, double U) {
            ...
            Mains) // hamowanie EP09   //TUHEX
        {
            ...
            MotorCurrent = -Max0R(MotorParam[0].fi * (Vadd / (Vadd + MotorParam[0].Isat) - MotorParam[0].fi0), 0) * n *
                           2.0 / DynamicBrakeRes;
...
MotorCurrent:=(U-Isf*Rz-Mn*fi*n+SQRT(Delta))/(2*Rz)
                        ...
                        if ((DynamicBrakeType == dbrake_switch) && (TrainType == dt_ET42)) { // z Megapacka
                            Rz = WindingRes + R;
        ...
        if (MotorCurrent > 0) {
            ...
            if ((FuzzyLogic(fabs(Im), Imax * 2, p_elengproblem) || FuzzyLogic(fabs(n), nmax * 1.11, p_elengproblem)))
    ...
    bool TMoverParameters::Attach(
        ...
        auto const couplercheck{
                (Enforce) || ((distance <= dEpsilon) && (coupler.type() != TCouplerType::NoCoupler) &&
        ...
        if ((EnginePowerSource.SourceType == TPowerSource::CurrentCollector) // tylko jeśli pantografujący
            ...
            { // zbiornik pantografu połączony ze zbiornikiem głównym - małą sprężarką się tego nie napompuje
              // Ra 2013-12: Niebugocław mówi, że w EZT nie ma potrzeby odcinać kurkiem
                ...
                PantPress =
                        clamp((10.0 * PantVolume) - 1.0, 0.0,
                              EnginePowerSource.CollectorParameters.MaxPress); // tu by się przydała objętość zbiornika
            }
    ...
    void TMoverParameters::UpdateBatteryVoltage(double dt) { // przeliczenie obciążenia baterii
        ...
        if ((BatteryVoltage > 0) && (EngineType != TEngineType::DieselEngine) &&
            ...
            auto const converteractive{
                    (Power110vIsAvailable) || ((EngineType == TEngineType::DieselElectric) && (true == Mains)) ||
    ...
    double TMoverParameters::EngineRPMRatio() const {
        ...
        return clamp(
                (EngineType == TEngineType::DieselElectric ? ((60.0 * std::fabs(enrot)) / DElist[MainCtrlPosNo].RPM)
    ...
    */
    ...
    double TMoverParameters::ComputeMovement(
            double dt, double dt1, const TTrackShape &Shape, TTrackParam &Track, TTractionParam &ElectricTraction,
        ...
        */
        ...
        auto const d{
                (EngineType == TEngineType::WheelsDriven ? dL * CabActive : // na chwile dla testu
                         dL)};
    ...
    double TMoverParameters::FastComputeMovement(
            double dt, const TTrackShape &Shape, TTrackParam &Track, TLocation const &NewLoc, TRotation const &NewRot) {
        int b;
    ...
    void TMoverParameters::ConverterCheck(double const Timestep) {
        ...
        if ((ConverterOverloadRelayStart == start_t::converter) && (false == (ConverterAllow && ConverterAllowLocal)) &&
    ...
    void TMoverParameters::WaterPumpCheck(double const Timestep) {
        ...
        // the breaker
        WaterPump.is_active =
                ((true == (Power24vIsAvailable || Power110vIsAvailable)) && (true == WaterPump.breaker) &&
    ...
    void TMoverParameters::OilPumpCheck(double const Timestep) {
        ...
        OilPump.is_active =
                ((true == (Power24vIsAvailable || Power110vIsAvailable)) && (false == Mains) &&
        ...
        auto const minpressure{
                OilPump.pressure_minimum > 0.f ? OilPump.pressure_minimum : 0.15f}; // arbitrary fallback value
        ...
        if (OilPump.pressure < OilPump.pressure_target) {
            ...
            OilPump.pressure = std::min<float>(
                    OilPump.pressure_target, OilPump.pressure + (enrot > 5.0 ? 0.05 : 0.035) * Timestep);
        }
        if (OilPump.pressure > OilPump.pressure_target) {
            OilPump.pressure = std::max<float>(
                    OilPump.pressure_target, OilPump.pressure - (enrot > 5.0 ? 0.05 : 0.035) * 0.5 * Timestep);
        }
    ...
    void TMoverParameters::MotorBlowersCheck(double const Timestep) {
        ...
        for (auto &fan: MotorBlowers) {
            ...
            if (revolutionstarget > 0.f) {
                ...
                fan.revolutions +=
                        clamp(revolutionstarget - fan.revolutions, speedincreasecap * -2, speedincreasecap) * Timestep;
            } else {
    ...
    void TMoverParameters::PantographsCheck(double const Timestep) {

        {
            ...
            auto const autostart{
                    valve.start_type == start_t::automatic || valve.start_type == start_t::manualwithautofallback};
            auto const manualcontrol{
                    valve.start_type == start_t::manual || valve.start_type == start_t::manualwithautofallback};
    ...
    bool TMoverParameters::IncMainCtrl(int CtrlSpeed) {
        ...
        if (MainCtrlPos < MainCtrlPosNo) {
            switch (EngineType) {
                ...
                case TEngineType::ElectricInductionMotor: {
                    if (CtrlSpeed > 1) {
                        OK = (IncMainCtrl(1) && IncMainCtrl(CtrlSpeed - 1)); // a fail will propagate up the recursion
    ...
    bool TMoverParameters::IncScndCtrl(int CtrlSpeed) {
        ...
        if ((DynamicBrakeFlag) && (TrainType == dt_ET42) && (CabActive != 0) && (IsMainCtrlNoPowerPos()) &&
            ...
            OK = DynamicBrakeSwitch(false);
                ...
                !((TrainType == dt_ET42) && ((Imax == ImaxHi) || ((DynamicBrakeFlag) && (MainCtrlPowerPos() > 0))))) {
    ...
    bool TMoverParameters::WaterHeaterBreakerSwitch(bool State, range_t const Notify) {
        ...
        */
        ...
        if (Notify != range_t::local) {
            SendCtrlToNext(
                    "WaterHeaterBreakerSwitch", (WaterHeater.breaker ? 1 : 0), CabActive,
                    (Notify == range_t::unit ? coupling::control | coupling::permanent : coupling::control));
        }
    ...
    bool TMoverParameters::CompartmentLightsSwitch(bool State, range_t const Notify) {
        ...
        if (Notify != range_t::local) {
            SendCtrlToNext(
                    "CompartmentLightsSwitch", (CompartmentLights.is_enabled ? 1 : 0), CabActive,
                    (Notify == range_t::unit ? coupling::control | coupling::permanent : coupling::control));
        }
    ...
    bool TMoverParameters::CompartmentLightsSwitchOff(bool State, range_t const Notify) {
        ...
        if (Notify != range_t::local) {
            SendCtrlToNext(
                    "CompartmentLightsSwitchOff", (CompartmentLights.is_disabled ? 1 : 0), CabActive,
                    (Notify == range_t::unit ? coupling::control | coupling::permanent : coupling::control));
        }
    ...
    void TMoverParameters::MainSwitch_(bool const State) {
        ...
        godot::UtilityFunctions::print(godot::String("MainSwitch_: state={0}").format(args));
        args.clear();
        args.push_back(MainSwitchCheck());
        godot::UtilityFunctions::print(godot::String("MainSwitch_: MainSwitchCheck={0}").format(args));

        if ((false == State) || (true == MainSwitchCheck())) {
            godot::UtilityFunctions::print(godot::String("MainSwitch_: oho 1"));

            if (true == State) {
                // switch on
                godot::UtilityFunctions::print(godot::String("MainSwitch_: oho 2"));
                if ((EngineType == TEngineType::DieselEngine) || (EngineType == TEngineType::DieselElectric)) {
                    // launch diesel engine startup procedure
                    dizel_startup = true;
                    godot::UtilityFunctions::print(godot::String("MainSwitch_: dizel_startup"));
                } else {
                    // additional check, as vehicles without pantographs won't fail relay checks earlier
                    Mains = true;
                    godot::UtilityFunctions::print(godot::String("MainSwitch_: Mains=true!"));
                }
            } else {
                godot::UtilityFunctions::print(godot::String("MainSwitch_: Ojoj, Mains=false"));
                Mains = false;
    ...
    bool TMoverParameters::UniversalBrakeButton(int button, int state) {
        ...
        if (TestFlag(
                    UniversalBrakeButtonFlag[0] & UniversalBrakeButtonFlag[1] & UniversalBrakeButtonFlag[2],
    ...
    void TMoverParameters::CompressorCheck(double dt) {
        ...
        auto const CompressorSpeedF{
                CompressorList[TCompressorList::cl_SpeedFactor][CompressorListPos] * CompressorSpeed};
        ...
        auto const compressorenable{
                (CompressorAllowLocal) && ((CompressorStart == start_t::automatic) || (CompressorAllow))};
        ...
        switch (CompressorPower) {
            case 3: {
                ...
                CompressedVolume += CompressorSpeedF * (2.0 * MaxCompressorF - Compressor) / MaxCompressorF *
                                    EngineRPMRatio() * dt *
    ...
    void TMoverParameters::UpdatePipePressure(double dt) {
        ...
        if (BrakeCtrlPosNo > 1) {
            ...
            if ((EngineType != TEngineType::ElectricInductionMotor)) {
                ...
                dpLocalValve =
                        LocHandle->GetPF(std::max(lbpa, LocalBrakePosAEIM), Hamulec->GetBCP(), ScndPipePress, dt, 0);
            } else
            ...
            bool lock_old =
                    ((BrakeHandle == TBrakeHandle::FV4a) // old complex condition based on assumptions
    ...
    void TMoverParameters::ComputeMass() {
        ...
        if (ToLower(LoadQuantity) == "tonns") {
            ...
            loadtypeunitweight =
                    (lookup != simulation::Weights.end() ? lookup->second : 1000.f); // legacy default unit weight value
        }
        ...
        */
        ...
        if (true == SlippingWheels) {
            ...
            temp_nrot = ComputeRotatingWheel(
                    Fwheels - Sign(nrot * M_PI * WheelDiameter - V) * Adhesive(RunningTrack.friction) * TotalMassxg, dt,
                    nrot);
    ...
    double TMoverParameters::BrakeForceR(double ratio, double velocity) {
        ...
        if (MBPM > 2) {
            ...
            if (MaxBrakePress[1] > 0.1) {
                ...
                if ((BrakeDelayFlag & bdelay_R) && (BrakeMethod % 128 != bp_Cosid) && (BrakeMethod % 128 != bp_D1) &&
                    (BrakeMethod % 128 != bp_D2) && (Power < 1) && (velocity < 40))
                    ratio = ratio / 2;
    ...
    */
        ...
        switch (EngineType) {
            case TEngineType::DieselElectric: {
                if ((true == Mains) && (true == FuelPump.is_active)) {
                    ...
                    dizel_fill = clamp(
                            0.2 + 0.35 * (tmp - enrot) + 0.5 * (std::fabs(Im) / DElist[MainCtrlPosNo].Imax), 0.05, 1.0);
                } else {
                    tmp = 0.0;
                    dizel_fill = 0.0;
                }
                ...
                if (enrot != tmp) {
                    enrot = clamp(
                            enrot + (dt / dizel_AIM) *
                                            (enrot < tmp ? 1.0 : -1.0 * dizel_RevolutionsDecreaseRate), // NOTE:
                                                                                                        // revolutions
                                                                                                        // typically
                                                                                                        // drop faster
                                                                                                        // than they
        ...
        */
                    ...
                    switch (RVentType) {
                        ...
                        case 1: { // manual
                            if ((true == RVentForceOn) ||
                                ((DirActive != 0) && (RList[MainCtrlActualPos].R > RVentCutOff))) {
                        ...
                        case 2: { // automatic
                            ...
                            if ((std::fabs(Itot) > RVentMinI) && (RList[MainCtrlActualPos].R > RVentCutOff)) {

                                RventRot += (RVentnmax * std::min(1.0, ((motorcurrent / NPoweredAxles) / RVentMinI)) *
                                                     motorcurrent / ImaxLo -
                                             RventRot) *
                                            RVentSpeed * dt;
        ...
        switch (EngineType) {
            ...
            case TEngineType::ElectricSeriesMotor: {
                ...
                EventFlag |=
                        ((true == Mains) && ((false == NoVoltRelay) || (false == OvervoltageRelay)) &&
                         (MainSwitch(
            ...
            case TEngineType::ElectricInductionMotor: {
                ...
                if ((Mains)) {
                    ...
                    if ((std::max(GetTrainsetHighVoltage(), PantographVoltage) <
                        ...
                        MainSwitch(
                                false,
                                (TrainType == dt_EZT ? range_t::unit : range_t::local)); // TODO: check whether we need
                                                                                         // to send this EMU-wide
                    }
                }
                break;
            ...
            switch (EngineType) {
                ...
                case TEngineType::ElectricSeriesMotor: {
                    ...
                    if ((DynamicBrakeType == dbrake_automatic) && (DynamicBrakeFlag)) {
                        ...
                        if (((Vadd + fabs(Im)) > TUHEX_Sum + TUHEX_Diff) || (Hamulec->GetEDBCP() < 0.25) ||
                            (TUHEX_ResChange) || (TUHEX_StageActual == 0 && TUHEX_Stages > 0)) {
                            Vadd -= 500.0 * dt;
                            if (Vadd < TUHEX_MinIw) {
                                Vadd = 0;
                                DynamicBrakeFlag = false;
                    ...
                    else if ((TrainType == dt_EZT) && (Imin == IminLo) && (ScndS)) // yBARC - boczniki na szeregu
                                                                                   // poprawnie
                        Itot = Im;
                    else
                ...
                case TEngineType::DieselElectric: // youBy
                {
                    ...
                    if (ShuntMode) {
                        ...
                        if (fabs(Im) > tempImax) { // nie ma nadmiarowego, tylko Imax i zwarcie na pradnicy
                            Ft = Ft / Im * tempImax;
                            Im = tempImax;
                        }
                        ...
                        if (Im > 0) {
                            ...
                            if (true == Flat) {
                                ...
                                // Pitagorasa
                                EngineVoltage =
                                        std::sqrt(std::fabs(square(tempUmax) - square(tempUmax * Im / tempImax))) *
                                                (tempMCP - 1) +
                                        (1.0 - Im / tempImax) * tempUmax * (tempMCPN - tempMCP);
                                EngineVoltage /= (tempMCPN - 1);
                                EngineVoltage = clamp(EngineVoltage, Im * 0.05, (1000.0 * tmp / std::fabs(Im)));
                            }
                        }
                    ...
                    if ((false == StLinFlag) || (ShuntMode)) {
                        ...
                        if (AutoRelayFlag) {
                            ...
                            switch (RelayType) {
                                ...
                                case 41: {
                                    if ((ScndCtrlPos < ScndCtrlPosNo) && (MainCtrlPos == MainCtrlPosNo) &&
                ...
                case TEngineType::ElectricInductionMotor: {
                    if (true == Mains && !SecuritySystem.is_engine_blocked()) {
                        ...
                        if ((false == Doors.instances[side::left].is_closed) ||
                            ...
                            (Doors.permit_needed &&
                             (Doors.instances[side::left].open_permit || Doors.instances[side::right].open_permit))) {
                            DynamicBrakeFlag = true;
                        } else if (
                                ((edBCP < 0.25) && (false == localbrakeactive) && (AnPos < 0.01)) ||
                        ...
                        if ((DynamicBrakeFlag)) {
                            ...
                            if (eimv[eimv_Fmax] * Sign(V) * DirAbsolute < -1) {
                                PosRatio = -Sign(V) * DirAbsolute * eimv[eimv_Fr] /
                                           (eimc[eimc_p_Fh] * Max0R(edBCP, Max0R(0.01, Hamulec->GetEDBCP())) /
                                            MaxBrakePress[0]);
                        ...
                        eimv_pr += Max0R(Min0R(PosRatio - eimv_pr, 0.02), -0.02) * 12 *
                                   (tmp /*2{+4*byte(PosRatio<eimv_pr)*/) * dt; // wartość zadana/procent czegoś
                        ...
                        if ((eimv_pr < 0)) {
                            ...
                            eimv[eimv_Pmax] = std::min(
                                    eimc[eimc_p_Pmax], 0.001 * EngineVoltage * (eimc[eimc_p_Imax] - eimc[eimc_f_I0]) *
                        ...
                        eimv[eimv_FMAXMAX] =
                                0.001 *
                                square(std::min(
                                               1.0, eimv[eimv_fkr] / std::max(
                                                                             fabs(enrot) * eimc[eimc_s_p] +
                                                                                     eimc[eimc_s_dfmax] * eimv[eimv_ks],
                                                                             eimc[eimc_s_dfmax])) *
                                       f_cfu / eimc[eimc_s_cfu]) *
                                (eimc[eimc_s_dfmax] * eimc[eimc_s_dfic] * eimc[eimc_s_cim]) * Transmision.Ratio *
                                NPoweredAxles * 2.0 / WheelDiameter;
                        if ((eimv_pr < 0)) {
                            ...
                            if (EIMCLogForce)
                                pr = -log(1 - 4 * pr) / log(5);
                            eimv[eimv_Fr] = -Sign(V) * (DirAbsolute)*std::min(
                                                               eimc[eimc_p_Ph] * 3.6 / (Vel != 0.0 ? Vel : 0.001),
                            ...
                            eimv[eimv_Fful] =
                                    Min0R(Min0R(3.6 * eimv[eimv_Pmax] / Max0R(Vel, 1),
                        ...
                        eimv[eimv_Ic] = (eimv[eimv_fp] - DirAbsolute * enrot * eimc[eimc_s_p]) * eimc[eimc_s_dfic] *
                                        eimv[eimv_pole];
                        ...
                        eimv[eimv_Ipoj] = (eimv[eimv_Ic] * NPoweredAxles * InvertersRatio * eimv[eimv_U]) /
                                          ...
                                          eimc[eimc_f_I0];
                        eimv[eimv_Pm] =
                                DirActive * eimv[eimv_M] * NPoweredAxles * InvertersRatio * enrot * Pirazy2 / 1000;
                        ...
                        if ((RlistSize > 0) && ((std::fabs(eimv[eimv_If]) > 1.0) && (tmpV > 0.0001))) {
                            ...
                            InverterFrequency = (tmpV - DElist[i].RPM) /
    ...
    bool TMoverParameters::RelayReset(int const Relays, range_t const Notify) {
        ...
        if (TestFlag(Relays, relay_t::maincircuitground)) {
            if (((EngineType == TEngineType::ElectricSeriesMotor) || (EngineType == TEngineType::DieselElectric)) &&
                ...
                (IsMainCtrlNoPowerPos()) && (ScndCtrlPos == 0) && (DirActive != 0) && (!TestFlag(EngDmgFlag, 1))) {
    ...
    bool TMoverParameters::AutoRelayCheck(void) {
        ...
        auto const ARFASI{
                ((false == AutoRelayFlag) || ((RList[MainCtrlActualPos].AutoSwitch) && (fabs(Im) < Imin)) ||
            ...
            if (StLinFlag) {
                if ((RList[MainCtrlActualPos].R == 0) && ((ScndCtrlActualPos > 0) || (ScndCtrlPos > 0)) &&
                    ...
                    if ((MainCtrlActualPos < (sizeof(RList) / sizeof(TScheme) -
                        ...
                        if ((RList[MainCtrlPos].R == 0) && (MainCtrlPos > 0) && (MainCtrlPos != MainCtrlPosNo) &&
                            (FastSerialCircuit == 1)) {
                            ...
                            if ((TrainType == dt_ET22) && (MainCtrlPos > 1) &&
                                ((RList[MainCtrlActualPos].Bn < RList[MainCtrlActualPos + 1].Bn) || (DelayCtrlFlag))) {
    ...
    bool TMoverParameters::MotorConnectorsCheck() {
        ...
        auto const connectorsoff{
                (false == Mains) || (true == FuseFlag) || (true == StLinSwitchOff) || (DirActive == 0)};
    ...
    bool TMoverParameters::OperatePantographValve(end const End, operation_t const State, range_t const Notify) {
        ...
        if (Notify != range_t::local) {
            SendCtrlToNext(
                    ...
                    static_cast<double>(
                            0x80 * (End == end::front ? 0 : 1) + 0x40 * (CabActive != -1 ? 1 : 0) +
    ...
    void TMoverParameters::CheckEIMIC(double dt) {
        ...
        switch (EIMCtrlType) {
            case 0:
                ...
                if (EIMCtrlAdditionalZeros || EIMCtrlEmergency) {
                    if (eimic > 0.001)
                        eimic = std::max(
                                0.002, eimic * (double)MainCtrlPosNo / ((double)MainCtrlPosNo - offset) -
                                               offset / ((double)MainCtrlPosNo - offset));
                    if ((eimic < -0.001) && (BrakeHandle != TBrakeHandle::MHZ_EN57))
                        eimic = std::min(
                                -0.002, eimic * (double)LocalBrakePosNo / ((double)LocalBrakePosNo - multiplier) +
            ...
            case 3:
                if (UniCtrlIntegratedBrakePNCtrl) {
                    if ((UniCtrlList[MainCtrlPos].mode != BrakeCtrlPos) &&
                        (MainCtrlActualPos == MainCtrlPos)) // there was no move of controller, but brake only
                    {
                ...
                if ((MainCtrlActualPos != MainCtrlPos) || (LastRelayTime > InitialCtrlDelay)) {
                    eimic -=
                            clamp(-UniCtrlList[MainCtrlPos].SetCtrlVal + eimic, 0.0,
                                  (MainCtrlActualPos == MainCtrlPos
        ...
        auto const eimicpowerenabled{
                ((true == Mains) || (Power == 0.0)) && (!SpringBrake.IsActive || !SpringBrakeCutsOffDrive) &&
                (!LockPipe) && (DirAbsolute != 0)};
            ...
            if (true) {
                if ((!SpeedCtrlUnit.Standby)) {
                    ...
                    bool retarder_not_work =
                            (EngineType != TEngineType::DieselEngine) || (Vel < SpeedCtrlUnit.BrakeInterventionVel);
                    if (eSCP < -1.0) {
                    ...
                    if (fabs(eSCP) < 0.999) {
                        ...
                        double factorI =
                                eimicSpeedCtrlIntegral >= 0 ? SpeedCtrlUnit.FactorIpos : SpeedCtrlUnit.FactorIneg;
                        eimicSpeedCtrlIntegral =
                                clamp(eimicSpeedCtrlIntegral + factorI * eSCP * dt, -1.0 + eSCP, 1.0 - eSCP);
                    } else {
    ...
    bool TMoverParameters::dizel_AutoGearCheck(void) {
        ...
        auto const VelDown{
                ((MotorParam[ScndCtrlActualPos].fi0 != 0.0) && (eimic_real <= 0.0) ? MotorParam[ScndCtrlActualPos].fi0
    ...
    bool TMoverParameters::dizel_StartupCheck() {
        ...
        if ((false == FuelPump.is_active)) {
            ...
            if (false == FuelPump.is_active) {
                godot::UtilityFunctions::print("dizel_startup zerowany, bo pompka paliwa");
            }
            if ((EngineType == TEngineType::DieselEngine) && (RList[MainCtrlPos].R == 0.0)) {
                godot::UtilityFunctions::print("MainCtrlPos", MainCtrlPos);
                godot::UtilityFunctions::print("dizel_startup zerowany, bo RList[MainCtrlPos].R = 0.0");
            }
            dizel_startup = false;
            //        }
        }
        ...
        if ((false == OilPump.is_active) || (OilPump.pressure < OilPump.pressure_minimum)) {
            ...
            if (OilPump.start_type == start_t::manual) {
                // with manual pump control startup procedure is done only once per starter switch press
                godot::UtilityFunctions::print("dizel_startup zerowany, bo za malo oleju");
                dizel_startup = false;
            }
        }
        ...
        if (true == dizel_heat.PA) {
            ...
            godot::UtilityFunctions::print("dizel_startup zerowany, bo dizel_heat.PA");
            dizel_startup = false;
        }
    ...
    bool TMoverParameters::dizel_Update(double dt) {
        ...
        if ((true == dizel_ignition) && (LastSwitchingTime >= InitialCtrlDelay)) {
            ...
            godot::UtilityFunctions::print(
                    "dizel_startup=false, bo ignition=true i LastSwitchingTime>InitialCtrlDelay");
            dizel_startup = false;
                ...
                if (EIMCtrlType > 0) {
                    ...
                    if (eimic_real > 0.005 && !hydro_TC_Lockup) {
                        dizel_nreg_min = std::min(
                                dizel_nreg_min + 2.5 * dt, dizel_nmin_hdrive + eimic_real * dizel_nmin_hdrive_factor);
                    } else {
                    ...
                    if (dizel_vel2nmax_Table.size() > 0 && !hydro_TC_Lockup) {
                        dizel_nreg_max = std::min(
                                std::min(dizel_nreg_max, enrot) + dizel_nreg_acc * dt,
    ...
    double TMoverParameters::dizel_Momentum(
        ...
        if (enrot > 0) {
            if (dizel_Momentum_Table.size() > 1) {
                ...
                Moment = (dizel_Mmax -
                          (dizel_Mmax - dizel_Mnmax) * square((enrot - dizel_nMmax) / (dizel_nMmax - dizel_nmax))) *
                                 dizel_fill -
                         dizel_Mstand;
            }
            Mm = Moment;
            ...
            (fabs(dizel_n_old - enrot) > 0.1)) // slizga sie z powodu roznic predkosci albo przekroczenia momentu
        {
            ...
            if (TorqueC > TorqueL) {
                if (TorqueC > TorqueL + fabs(TorqueH)) {
                    ...
                    hydro_TC_nOut = enrot - (n - enrot) * (TorqueC - TorqueL) /
        ...
        } else // nie slizga sie (jeszcze)
        {
            ...
            gearMoment = Moment;
            enMoment = 0;
            double enrot_min = enrot - (Min0R(TorqueC, TorqueL + fabs(hydro_TC_TorqueIn)) - Moment) / dizel_AIM * dt;
            double enrot_max = enrot + (Min0R(TorqueC, TorqueL + fabs(hydro_TC_TorqueIn)) + Moment) / dizel_AIM * dt;
    ...
    void TMoverParameters::dizel_Heat(double const dt) {
        ...
        auto const PT =
                ((false == dizel_heat.water.is_cold) && (false == dizel_heat.water.is_hot) &&
        ...
        auto const obciazenie{
                engineon * ((dizel_heat.powerfactor * EnginePower / 950) + (Heating ? HeatingPower : 0) + 70)};
        auto const Qd{qs * Ge - obciazenie};
        // silnik oddaje czesc ciepla do wody chlodzacej, a takze pewna niewielka czesc do otoczenia, modyfikowane
        // przez okienko
        auto const Qs{
        ...
        auto const dTo{
                (dizel_heat.auxiliary_water_circuit ? ((dizel_heat.kfo * (dizel_heat.Ts - dizel_heat.To)) -
        ...
        /*
           if( typ == "SP45" )
           Qp = (float)( podgrzewacz and ( true == WaterPump.is_active ) and ( Twy < 55 ) and ( Twy2 < 55 ) ) *
           1000; else
           */
        ...
        if (true == dizel_heat.auxiliary_water_circuit) {
            ...
            dizel_heat.zaluzje2 =
                    (dizel_heat.water_aux.config.shutters ? (PTC2 == 1)
            ...
            auto const kf2{
                    kurek07 * ((dizel_heat.kw * (0.3 + 0.7 * zaluzje2)) * dizel_heat.rpmw2 +
        ...
        auto const kf{
                ...
                2};
        auto const dTs{
                (dizel_heat.auxiliary_water_circuit ? ((dizel_heat.kfs * (dizel_heat.Ts - dizel_heat.Tsr))) / (gw * Cw)
    ...
    void TMoverParameters::update_doors(double const Deltatime) {
        ...
        auto const localopencontrol{
                ...
                ((Doors.open_control == control_t::passenger) || (Doors.open_control == control_t::mixed))};
        auto const remoteopencontrol{
                (Doors.open_control == control_t::driver) || (Doors.open_control == control_t::conductor) ||
                (Doors.open_control == control_t::mixed)};
        auto const localclosecontrol{
                (false == Doors.remote_only) &&
                ((Doors.close_control == control_t::passenger) || (Doors.close_control == control_t::mixed))};
        auto const remoteclosecontrol{
                (Doors.close_control == control_t::driver) || (Doors.close_control == control_t::conductor) ||
        ...
        for (auto &door: Doors.instances) {
            ...
            door.is_open =
                    ...
                    ((false == Doors.step_enabled) || (door.step_position >= (Doors.step_range != 0.f ? 1.f : 0.f)));
            door.is_closed = (door.position <= 0.f) && (door.step_position <= 0.f);
            door.is_door_closed = (door.position <= 0.f);

            door.local_open =
                    door.local_open && (false == door.is_open) && ((false == Doors.permit_needed) || door.open_permit);
            ...
            auto const openrequest{
                    (localopencontrol && door.local_open) || (remoteopencontrol && door.remote_open) ||
            ...
            auto const closerequest{
                    (door.remote_close && remoteclosecontrol) || (door.local_close && localclosecontrol) ||
            ...
            door.step_unfolding =
                    ((Doors.step_range != 0.f) && (Doors.step_enabled) && (false == Doors.is_locked) &&
                     (door.step_position < 1.f) && (door.is_opening));
            door.step_folding =
        ...
        /*
        // the door are closed if their timer goes below 0, or if the vehicle is moving faster than defined
        threshold std::array<side, 2> const doorids { side::right, side::left }; for( auto const doorid : doorids )
        { auto const &door { Doors.instances[ doorid ] }; if( true == door.is_open ) { if( ( ( Doors.auto_velocity
        != -1.f ) && ( Vel > Doors.auto_velocity ) )
        || ( ( door.auto_timer != -1.f ) && ( door.auto_timer <= 0.f ) ) ) {
    ...
    */
    ...
    bool TMoverParameters::CheckLocomotiveParameters(bool ReadyFlag, int Dir) {
        ...
        switch (BrakeValve) {
            ...
            case TBrakeValve::K: {
                ...
                Hamulec = std::make_shared<TWest>(
                        MaxBrakePress[3], BrakeCylRadius, BrakeCylDist, BrakeVVolume, BrakeCylNo, BrakeDelays,
                        BrakeMethod, NAxles, NBpA);
        ...
        Handle->SetParams(
                Handle_AutomaticOverload, Handle_ManualOverload, Handle_GenericDoubleParameter1,
    ...
    void TMoverParameters::PutCommand(
            std::string NewCommand, double NewValue1, double NewValue2, const TLocation &NewLocation) {
    ...
    bool TMoverParameters::SetInternalCommand(
            std::string NewCommand, double NewValue1, double NewValue2, int const Couplertype) {
        bool SIC;
    ...
    bool TMoverParameters::SendCtrlToNext(
            std::string const CtrlCommand, double const ctrlvalue, double const dir, int const Couplertype) {
        bool OK;
    ...
    OK:=SendCtrlToNext(command,CValue1,CValue2);
        ...
        else if (Command == "EIMIC") // ElectricInductionMotor Integrated Control - propulsion and brakes in
                                     // relative values
        {
            eimic_real = CValue1;
            OK = SendCtrlToNext(Command, CValue1, CValue2, Couplertype);
        } else if (Command == "Brake") // youBy - jak sie EP hamuje, to trza sygnal wyslac...
        {
            ...
            */
            ...
            //  OK:=Power>0.01;
            switch (static_cast<int>(CValue1 * CValue2)) { // CValue2 ma zmieniany znak przy niezgodności sprzęgów
                case 1: {
                    CabActive = 1;
                    break;
                }
        ...
        {                                  // Ra: uwzględnić trzeba jeszcze zgodność sprzęgów
            ...
            OperatePantographValve(
                    static_cast<end>(swap ? 1 - inputend : inputend), static_cast<operation_t>(inputoperation),
                    range_t::local);
            OK = SendCtrlToNext(Command, CValue1, CValue2, Couplertype);
