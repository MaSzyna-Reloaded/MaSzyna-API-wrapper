...
namespace godot {
    void GeneratorPowerSource::_bind_methods() {
        BIND_PROPERTY_W_HINT(
            ...
            "type",
            "type",
            &GeneratorPowerSource::get_generator_type,
            &GeneratorPowerSource::set_generator_type,
            "generator_type",
            PROPERTY_HINT_ENUM,
            "ElectricSeriesMotor,DieselEngine,SteamEngine,WheelsDriven,Dumb,DieselElectric,DumbDE,ElectricInductionMotor,Main,None"
        )

        BIND_PROPERTY(Variant::FLOAT, "min_voltage", "voltage/min", &GeneratorPowerSource::get_min_voltage, &GeneratorPowerSource::set_min_voltage, "min_voltage");
        BIND_PROPERTY(Variant::FLOAT, "max_voltage", "voltage/max", &GeneratorPowerSource::get_max_voltage, &GeneratorPowerSource::set_max_voltage, "max_voltage");
        BIND_PROPERTY(Variant::FLOAT, "min_rpm", "rpm/min", &GeneratorPowerSource::get_min_rpm, &GeneratorPowerSource::set_min_rpm, "min_rpm");
        BIND_PROPERTY(Variant::FLOAT, "max_rpm", "rpm/max", &GeneratorPowerSource::get_max_rpm, &GeneratorPowerSource::set_max_rpm, "max_rpm");
    ...
    void GeneratorPowerSource::update_config(TPowerParameters &params, TMoverParameters &mover) const {
        ...
        gen.voltage_min = min_voltage / 60;
        gen.voltage_max = max_voltage / 60;
        // NOTE: for consistency the fiz file specifies  revolutions per minute
        gen.revolutions_min = min_rpm / 60;
        gen.revolutions_max = max_rpm / 60;
    }
