...
namespace godot {
    ...
    void TrainElectricEngine::_do_fetch_state_from_mover(TMoverParameters *mover, Dictionary &state) {
        ...
        state["power_source"] = train_controller_node->tpower_source_map.at(mover->EnginePowerSource.SourceType);
        state["accumulator/recharge_source"] = train_controller_node->tpower_source_map.at(mover->EnginePowerSource.RAccumulator.RechargeSource);
        ...
        state["power_cable/source"] = train_controller_node->tpower_type_map.at(mover->EnginePowerSource.RPowerCable.PowerTrans);
    ...
    void TrainElectricEngine::_do_update_internal_mover(TMoverParameters *mover) {
        ...
        mover->EnginePowerSource.SourceType = train_controller_node->power_source_map.at(power_source);
        ...
        switch (power_source) {
            case TrainController::POWER_SOURCE_INTERNAL: {
                ...
                mover->EnginePowerSource.PowerType = lookup != train_controller_node->power_type_map.end() ? lookup->second : TPowerType::NoPower;
            }
