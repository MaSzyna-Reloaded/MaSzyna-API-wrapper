                ...
                for (int chunk_x = first_x; chunk_x <= last_x; chunk_x++) {
                    ...
                    for (int chunk_z = first_z; chunk_z <= last_z && strip.size() >= 3; chunk_z++) {
                        const std::vector<ClipVertex> piece =
                                single_cell ? strip : clip_to_cell_range(strip, Vector3::AXIS_Z, chunk_z, p_chunk_size_m);
                        if (piece.size() < 3) {
                            continue;
                        }
                        ...
                        for (size_t corner = 1; corner + 1 < piece.size(); corner++) {
                            ...
                            const Vector3 area = (fan[1]->position - fan[0]->position)
                                                         .cross(fan[2]->position - fan[0]->position);
