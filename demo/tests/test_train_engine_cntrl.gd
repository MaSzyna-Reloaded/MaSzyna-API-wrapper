...
func test_defaults():
    ...
    assert_eq(engine.cntrl_auto_relay_mode, TrainEngine.AUTO_RELAY_NO)
    assert_false(engine.cntrl_coupled_controllers)
    assert_false(engine.cntrl_has_camshaft)
    assert_eq(engine.motor_blowers_start_mode, TrainEngine.START_MODE_MANUAL)
    assert_eq(engine.fuel_pump_start_mode, TrainEngine.START_MODE_MANUAL)
    assert_eq(engine.oil_pump_start_mode, TrainEngine.START_MODE_MANUAL)
    assert_eq(engine.water_pump_start_mode, TrainEngine.START_MODE_MANUAL)
...
func test_round_trip_and_update_without_crashing():
    ...
    engine.cntrl_eim_control_type = TrainEngine.EIM_CONTROL_TYPE_2
    engine.cntrl_auto_relay_mode = TrainEngine.AUTO_RELAY_YES
    ...
    engine.fuel_pump_start_mode = TrainEngine.START_MODE_AUTOMATIC
    engine.oil_pump_start_mode = TrainEngine.START_MODE_AUTOMATIC
    engine.water_pump_start_mode = TrainEngine.START_MODE_BATTERY
    await wait_idle_frames(2)

    assert_eq(engine.cntrl_main_controller_position_count, 5)
    assert_eq(engine.cntrl_auto_relay_mode, TrainEngine.AUTO_RELAY_YES)
    assert_true(engine.cntrl_has_camshaft)
    assert_eq(engine.fuel_pump_start_mode, TrainEngine.START_MODE_AUTOMATIC)
...
func test_cntrl_oil_start_is_applied_to_diesel_engine():
    ...
    assert_eq(engine.oil_pump_start_mode, TrainEngine.START_MODE_AUTOMATIC)
