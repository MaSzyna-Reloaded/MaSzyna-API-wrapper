...
namespace godot {
    void TrainWipers::_bind_methods() {
        ClassDB::bind_method(D_METHOD("switch_increase"), &TrainWipers::switch_increase);
        ClassDB::bind_method(D_METHOD("switch_decrease"), &TrainWipers::switch_decrease);
        BIND_PROPERTY(TrainWipers, Variant::FLOAT, angle);
        BIND_PROPERTY(TrainWipers, Variant::INT, default_position);
        BIND_PROPERTY(TrainWipers, Variant::INT, wiper_count);
        BIND_PROPERTY_W_HINT_RES_ARRAY(
                TrainWipers, Variant::ARRAY, positions, PROPERTY_HINT_TYPE_STRING, "WiperListItem");
    }

    void TrainWipers::_register_commands() {
    ...
    void TrainWipers::_unregister_commands() {
    ...
    void TrainWipers::_do_update_internal_mover(TMoverParameters *p_mover) {
    ...
    void TrainWipers::_set_switch_position(const int p_position) {
        switch_position = std::clamp(p_position, 0, std::max(0, static_cast<int>(positions.size()) - 1));
    }

    void TrainWipers::switch_increase() {
        _set_switch_position(switch_position + 1);
    }

    void TrainWipers::switch_decrease() {
        _set_switch_position(switch_position - 1);
    }

    // DynObj.cpp:4048-4115
    void TrainWipers::_do_process_mover(TMoverParameters *p_mover, const double p_delta) {
        if (positions.size() == 0) {
            return;
        }
    ...
    void TrainWipers::_do_fetch_config_from_mover(TMoverParameters *p_mover, Dictionary &p_config) {
