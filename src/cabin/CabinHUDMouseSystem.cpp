...
namespace godot {
    ...
    RID CabinHUDMouseSystem::control_create(
        ...
        controls.insert(
                rid, Control{_pickable(p_mesh_instance_id, true), p_caption, p_hints, String(), p_pressed, p_released, p_increase,
                             p_decrease, p_drag, p_step_rotation, p_step_offset});
        ...
        if (mesh != nullptr) {
            ...
            controls[rid].small = mesh->get_global_transform().xform(mesh->get_aabb()).get_longest_axis_size() <
                                  SMALL_CONTROL_SIZE;
        }
        return rid;
    }
    ...
    CabinHUDMouseSystem::_pickable(const uint64_t p_mesh_instance_id, const bool p_with_children) {
        ...
        for (const ObjectID &id : meshes) {
            ...
            pickable.parts.push_back(Part{
                    id, mesh != nullptr && mesh->get_mesh().is_valid() ? mesh->get_mesh()->get_faces()
                                                                      : PackedVector3Array()});
        }
        return pickable;
    }
    ...
    bool CabinHUDMouseSystem::_hit_part(
            const Part &p_pickable, const Vector3 &p_from, const Vector3 &p_to, double &r_distance,
            Vector3 &r_point) {
    ...
    Vector2 CabinHUDMouseSystem::_increase_signs(const Control &p_control) const {
        ...
        const Vector3 after = parent_transform.xform(
                transform.xform(p_control.step_rotation.xform(point)) + p_control.step_offset);
