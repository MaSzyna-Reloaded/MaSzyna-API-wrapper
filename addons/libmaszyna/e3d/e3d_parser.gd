...
func _create_submodel(submodel):
    ...
    match submodel["type"]:
        ...
        E3DSubModel.SubModelType.GL_TRIANGLES:
            ...
            var generate_mesh_with_tangents = func(vertices, uvs, indices, normals):
                var st = SurfaceTool.new()
                st.begin(Mesh.PRIMITIVE_TRIANGLES)

                # Add vertices, UVs, and normals to the SurfaceTool
                for i in indices:
                    st.set_uv(uvs[i])
                    st.set_normal(normals[i])
                    st.add_vertex(vertices[i])

                # Generate tangents automatically
                st.generate_tangents()

                # Commit to create the mesh
                var _mesh = st.commit()
                return _mesh

            var mesh = generate_mesh_with_tangents.call(
                submodel["vertices"],
                submodel["uvs"],
                submodel["indices"],
                _normals
