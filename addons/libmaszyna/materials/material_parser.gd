...
func parse(model_path, material_file) -> MaszynaMaterial:
    ...
    if file:
        ...
        mat.albedo_texture_path = data.get("texture_diffuse", data.get("texture1", ""))
        mat.normal_texute_path = data.get("texture_normalmap", data.get("texture2", ""))
    else:
        mat.albedo_texture_path = material_file
        push_error("Material not found: " + model_path + ":" + material_file)
    return mat
