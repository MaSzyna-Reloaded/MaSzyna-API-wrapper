...
func test_defaults():
    ...
    assert_eq(heating.heating_generator_engine, VehicleEngine.MAIN)
    assert_eq(heating.heating_generator_min_rpm, 0.0)
    assert_eq(heating.heating_generator_max_rpm, 0.0)
    assert_eq(heating.heating_power_cable_type, VehicleController.POWER_TYPE_ELECTRIC)
    assert_eq(heating.heating_max_voltage, 0.0)

func test_generator_source_updates_without_crashing():
    heating.heating_source = VehicleController.POWER_SOURCE_GENERATOR
...
func test_power_cable_source_updates_without_crashing():
    heating.heating_source = VehicleController.POWER_SOURCE_POWERCABLE
    heating.heating_power_cable_type = VehicleController.POWER_TYPE_ELECTRIC
