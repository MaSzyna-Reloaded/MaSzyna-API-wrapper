...
func test_defaults_match_original_mover():
    ...
    assert_eq(brake.cntrl_brake_delays, TrainBrake.BRAKE_DELAY_GP)
    assert_eq(brake.cntrl_brake_op_modes, TrainBrake.BRAKE_OP_MODE_PNEPMED)
    assert_eq(brake.cntrl_brake_handle_type, TrainBrake.BRAKE_HANDLE_TYPE_FV4A)
    assert_eq(brake.cntrl_local_brake_handle_type, TrainBrake.BRAKE_HANDLE_TYPE_FD1)
    assert_eq(brake.cntrl_anti_skid_brake_type, TrainBrake.ANTI_SKID_BRAKE_MANUAL)
    assert_eq(brake.cntrl_local_brake_type, TrainBrake.LOCAL_BRAKE_TYPE_PNEUMATIC)
...
func test_round_trip_and_update_without_crashing():
    brake.cntrl_brake_system = TrainBrake.BRAKE_SYSTEM_ELECTRO_PNEUMATIC
    brake.cntrl_brake_ctrl_position_count = 8
    brake.cntrl_brake_delays = TrainBrake.BRAKE_DELAY_GPR_MG
    brake.cntrl_brake_op_modes = TrainBrake.BRAKE_OP_MODE_PN
    brake.cntrl_brake_handle_type = TrainBrake.BRAKE_HANDLE_TYPE_KNORR
    brake.cntrl_local_brake_handle_type = TrainBrake.BRAKE_HANDLE_TYPE_WESTINGHOUSE
    brake.cntrl_anti_skid_brake_type = TrainBrake.ANTI_SKID_BRAKE_AUTOMATIC
    brake.cntrl_local_brake_type = TrainBrake.LOCAL_BRAKE_TYPE_HYDRAULIC
    brake.cntrl_manual_brake_present = false
    brake.cntrl_dynamic_brake_type = TrainBrake.DYNAMIC_BRAKE_AUTOMATIC
    ...
    assert_eq(brake.cntrl_brake_system, TrainBrake.BRAKE_SYSTEM_ELECTRO_PNEUMATIC)
    assert_eq(brake.cntrl_brake_ctrl_position_count, 8)
    assert_eq(brake.cntrl_brake_delays, TrainBrake.BRAKE_DELAY_GPR_MG)
    assert_eq(brake.cntrl_dynamic_brake_type, TrainBrake.DYNAMIC_BRAKE_AUTOMATIC)
