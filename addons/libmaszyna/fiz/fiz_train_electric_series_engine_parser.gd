...
func _parse_circuit(kv: Dictionary, context: FizImportContext) -> void:
    ...
    if kv.has("ImaxLo"):
        node.imax_low = FizLineUtil.get_int(kv, "ImaxLo")
    if kv.has("ImaxHi"):
        node.imax_high = FizLineUtil.get_int(kv, "ImaxHi")
    if kv.has("IminLo"):
        node.imin_low = FizLineUtil.get_int(kv, "IminLo")
    if kv.has("IminHi"):
        node.imin_high = FizLineUtil.get_int(kv, "IminHi")
    if kv.has("TUHEX_Sum"):
        node.tuhex_sum = FizLineUtil.get_float(kv, "TUHEX_Sum")
    if kv.has("TUHEX_Diff"):
        node.tuhex_diff = FizLineUtil.get_float(kv, "TUHEX_Diff")
    if kv.has("TUHEX_MaxIw"):
        node.tuhex_max_current = FizLineUtil.get_float(kv, "TUHEX_MaxIw")
    if kv.has("TUHEX_MinIw"):
        node.tuhex_min_current = FizLineUtil.get_float(kv, "TUHEX_MinIw")
    if kv.has("TUHEX_Stages"):
        node.tuhex_stages = FizLineUtil.get_int(kv, "TUHEX_Stages")
    if kv.has("TUHEX_Sum1"):
        node.tuhex_sum_1 = FizLineUtil.get_float(kv, "TUHEX_Sum1")
    if kv.has("TUHEX_Sum2"):
        node.tuhex_sum_2 = FizLineUtil.get_float(kv, "TUHEX_Sum2")
    if kv.has("TUHEX_Sum3"):
        node.tuhex_sum_3 = FizLineUtil.get_float(kv, "TUHEX_Sum3")
...
func _parse_rlist_header(kv: Dictionary, context: FizImportContext) -> void:
    ...
    match vent_str:
        "automatic": node.fan_type = TrainElectricSeriesEngine.FAN_TYPE_AUTOMATIC
        "yes": node.fan_type = TrainElectricSeriesEngine.FAN_TYPE_YES
    if vent_str == "automatic" or vent_str == "yes":
        if kv.has("RVentnmax"):
            node.fan_max_rpm = FizLineUtil.get_float(kv, "RVentnmax") / 60.0
        if kv.has("RVentCutOff"):
            node.fan_cutoff_resistance = FizLineUtil.get_float(kv, "RVentCutOff")
    if kv.has("RVentMinI"):
        node.fan_min_current = FizLineUtil.get_float(kv, "RVentMinI")
    if kv.has("RVentSpeed"):
        node.fan_speed = FizLineUtil.get_float(kv, "RVentSpeed")
