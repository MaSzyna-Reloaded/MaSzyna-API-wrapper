...
func test_defaults():
    ...
    assert_eq(train.cntrl_converter_start_mode, VehicleController.START_MODE_MANUAL)
...
func test_round_trip_and_update_without_crashing():
    ...
    train.cntrl_converter_start_mode = VehicleController.START_MODE_AUTOMATIC
    train.cntrl_converter_start_delay = 2.0
    ...
    assert_eq(train.cntrl_converter_start_mode, VehicleController.START_MODE_AUTOMATIC)
