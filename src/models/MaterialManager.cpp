...
namespace godot {
    ...
    MaterialManager::MaterialManager() :
        ...
        mutex(memnew(Mutex)), parser(memnew(MaterialParser)), user_settings_node(nullptr) {
    ...
    MaterialManager::~MaterialManager() {
        ...
        // Ref and Dictionary clean themselves up
    }
    ...
    MaterialManager::load_texture(const String &model_path, const String &material_name) {
        ...
        // Note: checking if _unknown_texture "is null" via get_image() might be what was intended,

        // but Ref<ImageTexture> _unknown_texture starts as valid object if instantiated.

        // The original code was checking if it has content?

        // "if (_unknown_texture->is_null())" in original code was likely checking if the Ref IS null.


        // Let's assume we want to load the fallback if it's not loaded yet.

        // But here we are just checking if we need to load it.

        // Since we changed _unknown_texture to be a member Ref, it's null by default unless instantiated.


        // Actually, we should try to load the fallback once.
